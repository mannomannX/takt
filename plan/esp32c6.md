...
| 5 | **fertig 2026-09-17.** `takt-conformance/tests/board_esp32c6.rs` (nur mit `TAKT_ESP32C6_PORT=COM4`): 37 Programme des Differentialkorpus laufen auf dem Chip, je 60 Ticks, Trace über USB-Serial-JTAG, **0 Abweichungen** gegen den Interpreter — Monitore (47), `map`-Iteration (42), SHA-256 (39), Ströme und `sim`-gekoppelte Modelle (23–27, 49–55) eingeschlossen. Ausgelassen, weil der MCU-Rahmen sie nicht trägt: geplante Ausgaben (28), Systemkanäle (32, 34), Jobs (40). Dafür bekam der Rahmen Natives, Ströme, `sim`-Bindungen, Monitore, Floats/Arrays/vorzeichenlose Werte im Trace und schwache Treiber-Defaults (`takt_out_*`). Befund FB-194: Der Rahmen bemaß seine Zustandspuffer aus einer Schranke statt aus dem Struct des Codegens; `39_sha256` schrieb darüber hinaus, und die Stack-Wache des Boards fing es — auf dem Wirt blieb es unsichtbar. Nachtrag 2026-09-24 (FB-266): Der USB-Serial-JTAG kann sich festfahren — einmal der JTAG-Teil nach einem Trace-Überlauf (FB-264), einmal der Empfangsendpunkt der Konsole nach einem Testlauf — und der Test hing dann statt zu scheitern. Jetzt liest `capture` mit harter Frist in einem Hilfsthread, fragt ohne `takt end` den Tickzähler `g_tick` über JTAG (stehendes Programm oder verstummte Konsole, auch mitten im Lauf) und lässt das Board sein USB-Gerät neu anmelden: Wunsch `TAKT` in LP_AON STORE0 (oder in der Konsole vor einem RTS-Reset), Reset, das Board setzt sich über den RTC-Watchdog auf RTC-Ebene zurück — wie der EN-Pin, mitsamt USB-Serial-JTAG-Block —, und der Host zählt neu. Der frühere Weg (Pull-ups 250 ms weg, Block per PCR zurückgesetzt) löste den Konsolen-Wedge, nicht den JTAG-Wedge. Von Hand: `probe-rs write --chip esp32c6 b32 0x600b1000 0x54414b54` und `probe-rs reset`. Ein steckender JTAG-Teil sitzt nicht im USB-Block: Eine Neuanmeldung mit Blockreset holt ihn nicht zurück (2026-09-25 nachgewiesen). Nach Espressif (openocd-esp32 #316) setzt nur ein harter Reset über EN auch den USB-Serial-JTAG-Block zurück; darum ist die Neuanmeldung jetzt ein Reset auf RTC-Ebene über den RTC-Watchdog, ausgelöst über STORE0 per JTAG oder `TAKT` in der Konsole vor einem RTS-Reset. Beide Wege sind belegt, und der Reset holt den JTAG-Wedge zurück (fünf von fünf, 2026-09-25). Auslöser des Wedges sind 500-µs-Läufe, die über RTS gestartet wurden; die Suite startet über probe-rs und ist nicht betroffen (FB-264, Versuchsreihe). **Regel seit 2026-09-25 (FB-270):** Die Board-Suite läuft vor jedem Commit an Runtime, Rahmen oder Codegen; Abbilder liegen im Zwischenspeicher (`takt-board-images` im Temp-Verzeichnis), ein zweiter Lauf braucht 9 von 9 in 210 s statt 9 von 9 in 364 s. Der Wirt deckt seit FB-273 den virtuellen Schlaf und die Commit-Folge beider Rahmen ab (`virtual_sleep_is_invisible`). Das Verstummen hing an der Last (FB-267): Die Leitung setzte `wr_done` auch bei belegtem Endpunkt und schrieb nach `wr_done` weiter, sobald `serial_in_ep_data_free` es erlaubte — unter Last trog das Bit. Jetzt ein Paket nach dem anderen (höchstens 63 Byte, das nächste erst nach dem Rohflag „abgeholt“), und das Abholen weckt den Kern, der den Ring zwischen den Ticks leert (FB-264): 46_matrices mit 250 Byte je Tick läuft dreimal in Folge abweichungsfrei, Korpus 80 ist zurück im Board-Korpus. `59_persist_idle` war ein Codegen-Fehler (FB-268, `_advance` rückte nur den ersten Zeitzähler vor), behoben; 51, 53 und 55 fehlte in der MCU-Initialisierung der Strom-Commit (FB-269), behoben. Suite 2026-09-25 danach: 8 von 8 Tests in 374 s, alle 49 Korpusprogramme ohne Abweichung; davor: 7 von 8 Tests in 364 s, 41 von 49 Korpusprogrammen ohne Abweichung, kein Verstummen; 51, 53 und 55 mit 2 bis 3 Abweichungen durch einen Tick Verzug bei Stroemen (FB-269). |
