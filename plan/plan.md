...
| **M8 v1.2** ✔ (ohne Board 2) | Gescopte Instanzen (Lebenszyklus in `switch`, Spitzenlast über Konfigurationen, Overlay), `resume` (tiefe History, `saved` außerhalb des Overlays), Trigger mit `arm`/`disarm`/`fired`/`armed` (Knotenregel, Simulation mit `bound`), `capture<T, N>` als Stream-Element mit Armierung, Generics über Typen (Monomorphisierung, Fähigkeiten, azyklischer Instanziierungsgraph), Treiberstufe `port`, Anforderungsreferenzen, beschränkte QP-Löser; Prüfungen 52–55 aktiv | Inventur v1.2 grün; Konformität aktualisiert; Satz 9.4.1 mit gescopten Instanzen als Test (Aktivität aus der Konfiguration zu Tick-Beginn). **Stand 2026-09-22:** Schritte 1–19 und 21–23 fertig (`plan/m8.md` Abschnitt 8); Differentialkorpus 51 Programme, Interpreter und Codegen ohne Abweichung. Offen ist allein Schritt 20 (UART0-Treiber auf Board 2), weil er Hardware am Tisch verlangt; die Treiberstufe selbst ist mit `feedback/test_uart.takt` gegen ein Gerätemodell belegt |
...
**Entscheidung (2026-09-22): Die Treiberstufe steht in 12.10, nicht in
12.9.** `plan/m8.md` Nachzug 7.2 sah 12.9 vor; dort steht seit v0.2.3 die
verteilte Ausführung (v2), auf die Grammatik, Prüfung 58 und `plan/m9.md`
verweisen. Zwei Abschnitte umzunummerieren, um einen freizumachen, hätte
mehr Verweise gebrochen, als es geheilt hätte. Die Treiberstufe bekam
darum die nächste freie Nummer.

**Entscheidung (2026-09-17): M9 ist geplant — `plan/m9.md`.** Die
Referenz legt die Semantik von v2 fest (12.9, neun Regeln, Prüfung 58,
`Runtime(Node)`), nicht die Mechanik: Topologieformat, Link-Rahmen,
Handshake, Uhrendisziplin, Aufzeichnung je Knoten und der Knotenbuild
fehlen, und das Beispiel in 12.9 widerspricht Regel 1. `plan/m9.md` 0.2
hält die Abdeckung fest, Abschnitt 2 entscheidet — Ψ als
empfängerseitiger Ring, ein Link-Kern in `takt-rt-core` mit Transporten
als Treibern, Aufzeichnungen je Knoten, die allein abspielbar sind, der
Handshake als Atomaritätsinvariante — und teilt M9 in M9a (Verbund, 17
...
4. **Status und Dashboard.** Die Inventur führt je ID genau eine Status-Spalte mit fünf Werten: `offen`, `definiert`, `teilweise`, `fertig`, `rationale`. `definiert` heißt: die Syntax ist in `grammar/takt.ebnf` erfasst und `grammar/check_grammar.py` bestätigt Vollständigkeit gegen 2.2 und die Inventur; es gibt noch keinen Test. Was `fertig` heißt, legt die Kategorie über die Abnahme aus Punkt 2 fest — eine Grammatik-ID ist fertig, wenn Parse- und Formatter-Roundtrip grün sind; eine SEM-, PAR-, RULE- oder FN-ID, wenn ihr Interpreter-Test grün ist und ab M4 zusätzlich der differentielle Test gegen den nativen Code; eine SC-ID mit positivem *und* negativem Test; eine EX-ID, wenn das Beispiel auf jeder bis dahin unterstützten Zielklasse denselben Trace liefert. `teilweise` ist der ehrliche Zwischenstand, wenn eine Schicht steht und eine andere fehlt; welche, sagt der Commit, nicht die CSV. `rationale` tragen dauerhaft die Einträge, die nichts zu implementieren haben (CH, PRINC, DEC, STUFE) — sie sind Referenz für ADRs und werden nie abgehakt. Meilenstein-Exit = alle IDs des Meilensteins auf `fertig`. Abgehakt wird ausschließlich, was durch einen laufenden Test belegt ist.
   ...
   Eine zweite Spalte `Blockiert durch` nennt das Mittel, das zum Beleg fehlt — ein Messgerät, ein zweites Board, eine Zielklasse. Sie steht **neben** dem Status und nicht darin, weil beide verschiedene Fragen beantworten: Der Status sagt, wie weit die Arbeit ist, die Blockade, woran der Beleg hängt. Acht der heute blockierten Posten stehen auf `teilweise` — sie sind gebaut und warten auf die Messung; ein sechster Statuswert `blockiert` hätte das verdeckt. Leer heißt: Was fehlt, lässt sich mit dem schreiben, was da ist. Die Spalte ist damit die Antwort auf die Frage, was ohne Nachschub möglich bleibt — ein Filter, keine Einschätzung.
