...
| **M2 Ströme und Protokolle** | Streams mit Cursor und Byte-Ring, Muster → DFA (Alphabetklassen, ein DFA je Zustand), Handler, `until … matches`, Ausgabeströme, interne Streams, `reader`/`writer`, `layout` inkl. Bitfelder/Diskriminanten/`len_field`, `T!E`, Bereichsmuster, `at`/`pulse`/`cancel`, `samples` | 14.6 läuft im Interpreter gegen ein UART-Modell; Lemma 9.6.1 als Test; Prüfungen 17–21, 43, 45–47 |
...
3. **Dokumentation als Test.** Jeder Codeblock der Referenz wird in CI geparst; Beispiele werden ausgeführt; die Grammatikdatei `grammar/takt.ebnf` ist die Quelle des Grammatikabschnitts 2.3 (`check_grammar.py --sync` schreibt ihn).
...
1. Inventur kuratieren (1 055 IDs → gruppiert, Meilensteine bestätigt) und das Dashboard aufsetzen.
