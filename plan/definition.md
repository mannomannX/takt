...
system_item    := "tick" "=" duration_lit NEWLINE
                ...
                | "tcb_policy" "=" ( "curated_only" | ( "allowlist" | "reviewed" ) "(" IDENT { "," IDENT } ")" ) NEWLINE   (* Projekt-Natives 4.5, TCB-Manifest 9.5; v1.1, reviewed: v1.2 *)
...
attr           := "safe" "=" const_expr | "max_age" "=" duration_lit | "rate" "=" const_expr   (* @check 17, 28, 48 *)
                ...
                | "polling" "=" "unchecked"                          (* je Maschine, v1.2 *)   (* @check 59 *)
...
**Ranges sind Tick-Rand-Invarianten.** Eine Variable mit deklarierter Range hat diese Range an jedem Tick-Anfang (der Compiler prüft jede Zuweisung, 3.4); Channel- und Param-Ranges werden am Treiberrand erzwungen (3.5, 12.6) bzw. zur Ladezeit validiert (8.4). Die Analyse eines Ticks ist deshalb lokal — Straight-Line-Code plus beschränkte Schleifen — und braucht keine Fixpunktbildung über Ticks hinweg. `for`-Schleifen mit n·|Körper| unterhalb einer Schwelle werden abgerollt analysiert, sonst mit Widening auf die deklarierten Ranges der beschriebenen Variablen und anschließendem Narrowing: Der geweitete Kopf ist eine Invariante, und was ein Durchlauf von ihm bis zum Schleifenende trägt, vereint mit dem Eintritt, ist eine engere — höchstens drei absteigende Durchläufe, erst der letzte zählt Prüfungen. Neben den Intervallen führt die Analyse Differenzschranken `a − b ≤ c` zwischen Termen aus Variable, Feld und `len` (der Differenzanteil der Oktagone): Ein dominierender Vergleich trägt sie ein, eine Zuweisung an eine ihrer Variablen oder ein Methodenaufruf auf ihr löscht sie, eine Vereinigung behält die schwächere. So braucht `b[i]` hinter `if i >= b.len: break` keine Prüfung, und `b[k + 5]` keine hinter `if b.len < k + 7: …else:`. Ein `x != c` am Intervallrand schließt `c` aus. Akkumulatoren ohne Range erhalten implizite Prüfungen mit Warnung. Die Zahl impliziter Prüfungen ist eine Kennzahl im Report, aufgeschlüsselt nach ihrer Ursache — `Declared` (Zuweisung in eine engere Range), `Index` (Array-, `vec`- oder Slice-Zugriff), `Convert` (`as`-Konversion), `Arith` (Division, Overflow, Shift) und `NonFinite` (Gleitkomma: nicht endliches Ergebnis, `sqrt` unter null; 4.2). Die Summe allein führte in die Irre: Ein Protokollparser sammelt zwangsläufig viele `Index`-Prüfungen, die alle aus derselben fehlenden Schranke folgen, während eine einzige `Declared`-Prüfung in einer Regelschleife mehr wiegt. Die Aufschlüsselung sagt dem Techniker, *welche* Schranke fehlt, nicht nur *dass* eine fehlt. Zertifizierungsprojekte eskalieren die Kennzahl zu Fehlern (10).
...
**Oktagone (v1.1).** Die Analyse wird auf die Oktagon-Domäne (Miné) erweitert: Invarianten der Form `±x ± y <= c` in polynomieller Zeit — genau die Relationen `a < b` und `a - b <= c`, die Steuercode braucht (Differenzen korrelierter Sensoren, Abstände zu Sollwerten). Der Differenzanteil `a - b <= c` steht bereits (3.4, Differenzschranken); v1.1 ergänzt die Summen `x + y <= c` und den Abschluss über Ketten von Schranken. Die Schnittstelle der Analyse (implizite Prüfungen, Warnungen, Kennzahl) bleibt unverändert; nur die Präzision steigt. Ein SMT-Solver bleibt ausgeschlossen (Vorhersagbarkeit, Übersetzungszeit).
...
- `match` ist **erschöpfend**: Fehlt eine Variante ohne `case _:`, ist das ein Compile-Fehler — die konstruktive Form der Totalität für Summentypen. Speicherbedarf = größte Variante plus Diskriminante; im erzeugten Code eine 32-Bit-Diskriminante und je Feld der größten Variante ein 8-Byte-Fach (Ganzzahlen erweitert, Gleitkomma bitgleich, ungenutzte Fächer null), sodass `==` Diskriminante und Fächer vergleicht (11.2).
```
match classify(ev):
...
- **`inout`-Parameter** reiner Funktionen: `fn fill[const N](inout b: bytes<N>, x: u8)` ohne Rückgabetyp ist Zucker für eine Rückgabe — der Aufruf `fill(b, x)` als Anweisung steht für `b = fill(b, x)`, das Argument muss eine Stelle sein, und `-> T` ist neben `inout` nicht erlaubt (der Parameter ist die Rückgabe); die Funktion bleibt rein, die Zeigerübergabe übernimmt der Compiler (11.2). Ein Argument darf pro Aufruf nur einmal als `inout` gebunden werden und nicht zugleich als weiteres Argument erscheinen (kein Aliasing, statisch geprüft). Das ist die Zielform für C-Funktionen, die Puffer in place ändern. Sie machen Parsen und Zusammensetzen variabler Nutzlasten total und lesbar, ohne dynamischen Speicher: jede reale Nutzlast hat eine feste Obergrenze (Modbus 253 Byte, CAN-FD 64 Byte).
...
- Die Fähigkeiten sind Prädikate auf dem konkreten Typ, geprüft an der Aufrufstelle vor der Instanziierung: `pod` jeder Werttyp fester Größe ohne Handle (Skalare, `bool`, Enums, Records, Arrays und Puffer daraus); `eq` die POD-Typen mit erklärter Gleichheit — dieselbe Menge, die 3.9 als `map`-Schlüssel zulässt, also ohne Fließkomma; `ord` `int[U]`, `float[U]`, `time` und `bool`; `numeric` `int[U]` und `float[U]`; `integer` `int[U]`; `float` `float[U]`. Die Fähigkeit ist Vorbedingung und liefert die erste Meldung („`T` braucht `numeric`, `bool` hat es nicht"); Fehler im Rumpf nennen die Instanziierungsstelle.
- Geprüft wird je Instanz, nicht einmal gegen die Fähigkeit: Der Rumpf wird mit konkreten Typen gesenkt, weil die Einheitenalgebra sonst symbolisch über Typvariablen laufen müsste (`T * T` hätte die Einheit `U²`). Typvariablen gibt es in `fn` und `block`; `native` bleibt bei Konstanten, weil ein Native eine C-Signatur hat und je Typ ein Symbol das ABI vervielfachte. Records und Maschinen bleiben monomorph.
- Die Azyklizität ist eine Schranke auf der Instanziierungstiefe: Rekursion mit gleichen Argumenten fängt der Memo ab, wachsende Argumente (`f[T]` ruft `f[[1] T]`) meldet Prüfung 52, sobald die Tiefe acht übersteigt.
...
- **Chunk-Natives** für streamfähige Algorithmen führen ihren Zustand als Wert mit: `sha256_init() -> Sha256Ctx`, `sha256_update(ctx, chunk: bytes<4096>) -> Sha256Ctx`, `sha256_final(ctx) -> bytes<32>`. `Sha256Ctx` ist ein opakes Record fester Größe; die Kette über Ticks ist gewöhnlicher Takt-Code (`ctx = sha256_update(ctx, chunk)` je Chunk aus einem Stream), das Budget pro Tick bleibt statisch. `Sha256Ctx` ist im Prelude als Record `h: [8] u32, buf: bytes<64>, total: u64` vordefiniert; opak heißt: ein Programm liest und schreibt seine Felder nicht. Argumente und Ergebnisse, die keine Skalare sind — `bytes<N>` und Records —, überqueren die Grenze als Zeiger und Länge in der kanonischen Byteform (5.9); ein solches Ergebnis schreibt die Funktion in einen Puffer des Aufrufers. Die Deklaration muss der kuratierten Signatur entsprechen (Prüfung 31).
- **Jobs** für atomare Algorithmen, die in keinen Tick passen (Signaturprüfung: ECDSA P-256 braucht auf einem Cortex-M4 30–100 ms; Entschlüsselung; große Transformationen): `native job ecdsa_p256_verify(key: bytes<64>, digest: bytes<32>, sig: bytes<64>) -> bool with cost = 300, stack = 2048, duration = 120 ms, total`. Aufruf als Statement `job v = ecdsa_p256_verify(...)`; `v.done : bool` und `v.result : T!JobErr` sind *Inputs* (Teil von I_k). Semantik: Der Job startet im Tick des Statements (Kosten `cost` = Start, Argumente werden kopiert), läuft in der Runtime außerhalb der Schrittphase in einem vom Tick unterbrechbaren Kontext mit deklariertem `stack` (im `baremetal`-Profil in der Restzeit der Ticks, im `rtos`-Profil als eigener Task), und seine Fertigstellung erscheint als Input-Ereignis wie ein Sensorwert. Satz 9.4.1 gilt (Funktion der Inputs); Satz 9.4.4 gilt für das *Ergebnis* (bitidentisch), nicht für den Fertigstellungs-Tick — der wird aufgezeichnet und in der Simulation aus Aufzeichnung oder Modell (Default: `duration`) genommen. `duration` ist die deklarierte Worst-Case-Dauer und der Anker für `until v.done timeout …`. Je Maschine höchstens K_j gleichzeitige Jobs (statisch, Default 2); ein Fault-Übergang der Maschine bricht ihre laufenden Jobs ab (5.3); ein Job-Handle lebt in der Maschine und ist in dem Block sichtbar, in dem `job` steht. Vor der Fertigstellung trägt `v.result` den Wert `Err(PENDING)`, nach einem Abbruch `Err(CANCELLED)` (`JobErr`, offen). Die Fertigstellung steht im Trace als `job <maschine> <handle> done`; als Stimulus ersetzt diese Zeile den Tick des Modells. Die Argumente überqueren die Grenze zur Runtime in der kanonischen Byteform (5.9), das Ergebnis kommt in derselben Form zurück.
- v1: nur die kuratierte, mitgelieferte Menge (DSP-Kerne, Prüfsummen, Hashes, MACs). **Projekt-Natives** (v1.1): `native fn crc_custom(...) from "crypto.rs" with …` bindet eine Rust-Implementierung des Projekts ein; Pflicht sind die Konformitätstests aus 13.8 (Bit-Gleichheit über Targets, Panic-Freiheit unter Fuzzing, gemessener `stack`) und die Nennung im Lauf-Header, damit die erweiterte TCB sichtbar bleibt. Frei einbindbare Natives ohne diesen Prozess gibt es nicht: `system: tcb_policy = allowlist(f, …)` nennt jedes Projekt-Native beim Namen, Default ist `curated_only`; der Interpreter führt ein Projekt-Native nicht aus — die Simulation braucht eine kuratierte Funktion oder ein Modell. Kryptographie, die eine geprüfte Abhängigkeit braucht, steht in `takt-crypto` hinter einem Feature (`ecdsa_p256_verify` über `p256`), und das TCB-Manifest des Lauf-Headers nennt sie (12.5). Die Konformitätssuite (13.8) prüft Bit-Gleichheit über Targets und Panic-Freiheit unter Fuzzing.
...
3. **Wechsel.** Aktionen des Transitionsblocks, noch im verlassenen Zustand; dann `exit`-Blöcke der verlassenen Zustände (innen → außen) und `enter`-Blöcke der betretenen Zustände (außen → innen). Der kleinste gemeinsame Vorfahr bleibt aktiv; er liegt echt oberhalb des Zielzustands, ein Übergang auf sich selbst oder auf einen Vorfahren verlässt und betritt diesen also neu. Anders als in UML-Statecharts laufen die Aktionen vor den `exit`-Blöcken: Ein Fault in ihnen gehört zum Zustand, der den Übergang gewählt hat, und `exit` läuft genau einmal — auf dem Fault-Pfad.
...
Austritt des Zustands (switch, Schritt 2 = exit-Bloecke):
    ...
    scheitert einer, entfallen die restlichen, und der Fault wirkt nicht weiter;
...
- Budget (Schedulability): `Σ_c (Peak_c + Σ_m F_m,c) · c_target[c] ≤ T₀ − T_IO`, summiert über die Operationsklassen c ∈ {i32, i64, f32, f64, mem, call, native}, mit B_m,c dem statischen Operationsbudget einer Aktivierung je Klasse, F_m,c dem Fault-Pfad-Budget (beide 9.4.3), Σ_m F_m,c den Kosten der Abort-Phase (5.4) und `c_target[c]` der kalibrierten Kostentabelle des Targets (13.8; auf einem Kern ohne FPU liegt `c_target[f64]` zwei Größenordnungen über `c_target[i32]`; auf 32-Bit-Kernen liegt `c_target[i64]` über `c_target[i32]`, auf 64-Bit-Kernen sind beide gleich; Division, `fma` und `sqrt` haben eigene Gewichte, weil sie je nach Kern Hardwarebefehl oder Bibliotheksaufruf sind — sie zählen in ihrer Klasse und wiegen nie weniger als eine gewöhnliche Operation dort). `Peak = Σ_m B_m`, denn bei Phase 0 sind in Tick 0 alle Maschinen aktiv (mit gescopten Instanzen, 5.11, ist die Spitze das Maximum über Konfigurationen, weil Instanzen exklusiver Zustände nie gleichzeitig laufen); nur `phase`-Versatz kann die Spitze senken, was der Compiler exakt über die Hyperperiode prüft, wenn H ≤ 10⁶ (bei paarweise teilerfremden Perioden ist die Spitze nach dem Chinesischen Restsatz ohnehin Σ_m B_m). Verletzung ist ein Compile-Fehler mit Vorschlag (Periode erhöhen, Phase verschieben, Fault-Pfade verkleinern). `T_IO` ist die Zeit, die ein Tick außerhalb der Schrittfunktionen verbraucht: in den Schritten der Tickschleife (12.1) vor und nach `run_scheduled_machines` — Abtastung, Vertragsprüfung des Treiberrands, Stromzustellung, Cursor, Publish, geplante Ausgaben, Commit und Watchdog. **Nicht** dazu zählen `record_and_telemeter`, das 12.1 ausdrücklich „außerhalb der Semantik, nie blockierend“ führt, und `maybe_sleep`, das nur verbraucht, was ohnehin übrig ist. Die Abgrenzung ist zugleich die Messvorschrift für `takt bench` (13.8): Gemessen wird die Tickperiode minus der Zeit in den Schrittfunktionen, und was ein Recorder kostet, gehört in sein eigenes Budget — sonst verschöbe die Instrumentierung die Schranke, die sie beobachtet.

**Budget je Maschine.** `machine current_ctrl every 50 us with budget = {ram = 2 KiB}` deklariert, was eine Maschine verbrauchen darf. Ohne Deklaration prüft erst die Integration, ob die Summe passt — in einem Projekt mit mehreren Teams fällt die Überschreitung dann auf, wenn sie teuer ist. Mit Deklaration ist das Budget ein **Vertrag**, der lokal und sofort scheitert (Prüfung 62); dieselbe Logik trägt für wiederverwendbare Bausteine. `ram` ist der Speicher der Maschine in Byte, gerechnet wie in `takt size` (11.5, mit Overlay). `wcet` ist eine **Dauer je Aktivierung** und wird gegen `Σ_c (B_m,c + F_m,c) · c_target[c]` geprüft: Der Fault-Pfad zählt mit, weil er im selben Tick läuft (5.4), und `T_IO` geht nicht ein — es gehört dem Tick, nicht der Maschine. Je Aktivierung und nicht je Tick, weil `B_m` in 9.4.3 so definiert ist: Eine Maschine mit `n_m = 10` belastet nur jeden zehnten Tick, ihr Vertrag gilt aber für den Durchlauf. Ohne kalibrierte Kostentabelle ist die Prüfung nicht entscheidbar (13.8); sie meldet dann, was ihr fehlt, statt zu schweigen — eine Prüfung, die bei fehlender Eingabe stumm bleibt, ist von einer bestandenen nicht zu unterscheiden.

### 7.3 Überlauf zur Laufzeit
Überschreitet ein Tick trotz Budget die Periode (Treiberstörung, Cache-Effekte auf Linux), erzeugt die Runtime `Runtime(Overrun)` als Fault für alle Maschinen im nächsten Tick (`system: overrun = fault | alert`, Default `fault`; `alert` für unkritische Systeme — kein Logikanteil, die Semantik erzeugt nie einen Überlauf). Der Tick wird nie übersprungen; die logische Zeit bleibt konsistent, die physische Verzögerung wird protokolliert: Je Tick stehen `took` (Dauer des Schritts) und `drift` (Rückstand beim Beginn) in der Metazeile `time` der Aufzeichnung (12.5), und daraus folgen die vier Kennzahlen eines Laufs — verspätete Ticks (`drift ≥ T₀`), größter Rückstand, verlorene Perioden (die Summe der Zuwächse des Rückstands, nicht das Aufholen) und Überläufe. `takt timing` rechnet sie nach. Ein Journal-Schreibvorgang, der über die Frist hinaus dauert (12.3, blockierendes NVM), ist ebenfalls ein Überlauf; Prüfung 32 sagt vorher, ob das Programm ihn tragen kann.
...
Festlegungen der Umsetzung: Das Element von `fired` ist der Record aus den Captures des Musters und `.t` — die Felder, die 7.5 nennt, ohne den Inhalt des ausloesenden Elements: Ein `line` hat keine feste Byteform (8.6), die der Ring von `fired` braucht. Dieselbe Form traegt `event` im `then`, damit `event.n` und `f.n` nach `fired as f` dasselbe heissen. Eine Bindung auf `t.fired` traegt ihn direkt, ohne die Huelle, die ein gewoehnlicher Strom bekommt. Der Trigger führt einen eigenen Cursor auf seinem Quellstrom: Er wird mit Ereignisrate ausgewertet und gehört keiner Maschine, kann also nicht den Cursor seines Besitzers benutzen — der liest den Strom in der Regel gar nicht. Im Interpreter liegt die Trigger-Phase zwischen Zustellung und Schrittphase, damit die geplante Ausgabe im selben Tick in `sched` steht; `fired` wird wie jeder interne Strom im nächsten Tick sichtbar (8.6). Prüfung 55 verlangt zusätzlich `bound > 0` und, wo die Zeit die Form `event.t + d` mit konstantem `d` hat, `d >= bound`; `arm` aus einer zweiten Maschine ist ein Fehler, weil `armed` Zustand genau einer ist.
...
Sei `C` die aktive Kette, `loop(st)` der `loop:`-Block und `handlers(st)` die Handler der Ebene `st`, `trans(C)` die Transitionsliste in Outer-first-Reihenfolge, `fault_target(C, f)` das Fault-Ziel nach 5.3 (Check-spezifisch oder Zustand; in `FAULTED` wieder `FAULTED`), `switch(C, q, v)` der Konfigurationswechsel in fester Reihenfolge — Ziel unterhalb eines `resume`-Zustands ist der gespeicherte Pfad (5.12); gescopte Instanzen der verlassenen Zustände werden in Schritt 2 deaktiviert, die der betretenen in Schritt 3 initialisiert (5.11) —: (1) neue Konfiguration setzen — der kleinste gemeinsame Vorfahr liegt echt oberhalb von `q`, ein Übergang auf sich selbst oder einen Vorfahren verlässt und betritt ihn also neu —, (2) exit-Blöcke der verlassenen Zustände innen → außen ausführen — sie sehen noch die verlassene Konfiguration: ihre zustandslokalen Variablen sind gültig, `time_in_state` ist die Verweildauer des verlassenen Blatts; erst danach ist überlagerter Speicher der verlassenen Geschwister frei (11.2) —, (3) zustandslokale Variablen, Timer, `every`-Zähler und Bestätigungszähler `viol` der betretenen Zustände initialisieren, (4) enter-Blöcke außen → innen ausführen; scheitert eine Aktion in (2) bis (4), werden die restlichen übersprungen und der Ausgang ist FAULT — der Fault wird dann mit der bereits neuen Konfiguration behandelt; sonst NORMAL und `entered(C_old, C_new)` die Folge der neu betretenen Zustände unterhalb des kleinsten gemeinsamen Vorfahren, von außen nach innen.
...
*Beweis.* Im Modus ENTRY ist der einzige Nicht-NORMAL-Ausgang FAULT (Regel für `->`; Handler laufen dort nicht). Nach einer GOTO-Iteration folgen also nur FAULT-Iterationen. Jede FAULT-Iteration wechselt entlang einer Kante des Fault-Waldes, der azyklisch und endlich ist (statische Regel 5.3); `FAULTED` führt keinen Code aus und beendet die Schleife, und ein Fault, während die Konfiguration `FAULTED` ist, führt dorthin zurück (5.3) — auch er beendet sie. Jede Iteration führt endlich viele Statements aus (9.4.3). ∎
...
**Satz 9.4.3 (Statisches Kostenbudget).** Die Kosten sind Vektoren über den Operationsklassen c ∈ {i32, i64, f32, f64, mem, call, native}: `N(s) ∈ ℕ^7`, `cost(e)` zählt jede Operation in ihrer Klasse (eine Integer-Operation nach ihrer gewählten Darstellung in `i32` oder `i64` (3.4), eine f64-Multiplikation in `f64`, ein Array-Zugriff in `mem`, ein Aufruf in `call`, eine native Funktion mit ihrem deklarierten Vektor in `native`; eine Division, ein `fma` und eine Wurzel zählen in ihrer Klasse und dazu für sich, das Produkt zweier Matrizen als R·C `fma`-Ketten über K). Das Zeitbudget ist `Σ_c N_c · c_target[c]` mit der kalibrierten Tabelle des Targets, dazu je Division, `fma` und Wurzel, was ihr eigenes Gewicht über dem ihrer Klasse liegt (7.2, 13.8). Mit dem Kostenmodell
```
...
```
`sched` ist beschränkt (K_o), alle Operationen sind Funktionen von (Σ, I); die physische Ausführung zum Zeitpunkt T ist Treiberaufgabe; die Simulation wendet im Tick `ceil(T / T0)` an. `apply_scheduled(k)` läuft nach allen Schritten des Ticks und vor dem Commit (12.1): Fällt ein geplanter Wert in einen Tick, in dem der Output auch direkt zugewiesen wird, gewinnt der geplante.
...
| 32 | Schedulability mit Abort-Phase, klassenweise: `Σ_c (Peak_c + Σ F_m,c) · c_target[c] ≤ T₀ − T_IO` (7.2); `tick_source` existiert in der Hardware-Konfiguration. Blockierendes NVM (12.3): ein `persist`-Schreibvorgang kostet `⌈(nvm_erase_ns + 2·nvm_program_ns) / T₀⌉` Perioden — unter `overrun = fault` ohne `idle`-Zustand ein Fehler, mit `idle` ein Hinweis (Schreiben im Schlaf), unter `alert` eine Warnung; fehlt `nvm_blocking` auf einem Ziel mit `iram`, ist die Prüfung nicht entscheidbar | F |
...
- LLVM-Flags: `-O2` auf dem Wirt, `-Os` auf MCU-Zielen (die strikte FP-Form hängt an den Instruktionen, nicht an der Stufe), strikte FP (`-fno-fast-math`, `contract=off`), keine Speculation über Fault-Pfade hinweg; die Fault-Pfade sind `cold`.
- **Große Werte.** Werte über einer Schwelle (Default 64 Byte: Records, Arrays, Matrizen, `bytes<N>`, `line<N>`) werden intern per Zeiger übergeben und als Ergebnis in vom Aufrufer bereitgestellten Speicher geschrieben; auch `T?`/`T!E` darüber, `.or`, `map.get` und `c ? a : b` laufen über Plätze und `memcpy`, nie als SSA-Aggregat, weil der Optimierer daran nicht zu Ende kommt. Kopien entstehen nur bei Zuweisung, und nur dann als echte Kopie, wenn Ziel und ein Operand dieselbe Variable sind (`P = f(P)`); sonst wird direkt in das Ziel geschrieben. Das ist korrekt, weil die Sprache keine Referenzen hat: Jedes Objekt ist eine eigene benannte Speicherstelle, Überlappung ist ein syntaktischer Test. Die Wertsemantik der Sprache bleibt unangetastet.
...
block stopwatch()                   step(dt) -> Duration / start() / stop() / elapsed() -> Duration   # zaehlt dt, solange gestartet; ein Block liest kein now (5.7)
block pulse_counter()               step(edges: int) -> int                    # je Tick mit s.count gerufen; ein Block liest keinen Strom (5.7)
...
**Overlay über den Zustandsbaum.** „Mit Overlay" heißt: `mem(s) = eigen(s) + Summe der in s gescopten Instanzen + max über die XOR-Kinder`. Zustände, die einander ausschließen, teilen ihren Platz, und mit ihnen die Instanzen, die sie scopen (5.11) — eine Instanz in `FIRST` und eine in `SECOND` belegen zusammen so viel wie die größere allein. Latches werden nie überlagert: `persist`-Variablen liegen in Σ und überdauern jeden Zustandswechsel (5.9). Der Report nennt unter der Summe, wie viele Bytes die Überlagerung gegenüber der flachen Summe spart; die Zahl ist kein Posten, weil die Summe schon den überlagerten Stand zählt. Dieselbe Rekursion liefert die Spitzenlast für 9.4.3: `Peak` maximiert über die Konfigurationen statt über alle Maschinen zu summieren.

**Warum `gemessen` eine eigene Herkunft ist.** Der Posten *Flash (Code)* ist aus der MIR nicht ableitbar — wie viele Bytes eine Maschine wird, entscheidet der Codegen —, aber er steht auch in keiner Deklaration. Nach dem Linken ist er dennoch **exakt**: Die Sektionsgrößen des Objekts sind keine Schätzung. Ihn als `offen` zu führen wäre darum irreführend, und ihn `exakt` zu nennen verwischte, dass er aus einem anderen Schritt stammt als der Rest. Die Unterscheidung hat eine praktische Folge: `takt size` vor dem Link zeigt den Posten als `offen` und lässt ihn aus der Summe; mit einem Objekt zeigt es ihn als `gemessen` und rechnet ihn ein; `takt size --target NAME` übersetzt das Programm dafür selbst. Der Stack-Posten ist je Maschine ihr Schritt, darunter ihre tiefste Schleifen- oder Eintrittsfunktion, darunter der längste Pfad durch die gerufenen Funktionen (12.3); die Rahmen stehen im Prolog des Objekts, auf RISC-V samt `__riscv_save_N`, und eine überall eingebettete Funktion hat keinen eigenen. Der Bericht nennt den Pfad. Dasselbe gilt für die Konstanten, die der Link in `.rodata` legt. Von der Kalibrierung (13.8) unterscheidet sich das: Eine Profilreserve braucht Hardware und einen Lauf unter Last, eine Sektionsgröße nur den Linker. Für `baremetal`- und `rtos`-Profile ist die Summe gegen `ram`, `flash` und ggf. `iram` der Hardware-Konfiguration (8.10) zu prüfen, sobald diese vorliegt; Überschreitung ist ein Compile-Fehler, der die größten Posten mit Vorschlägen nennt (`expect_len`, kleinere `capacity`, `float = f32`, Matrizengröße). Das ist die konstruktive Form von „Speicherbudgets vor dem Kompilieren kennen": Der Compiler kennt sie.
...
Ein Lauf zeichnet auf: Logik-Hash, Bindungen, TCB-Manifest (9.5), Laufzeitprofil (12.8), Profil und Parametervektor, s0 der `persist`-Variablen, alle Input-Snapshots einschließlich Stream-Elementen mit Zeitstempeln und `free[o]`, Commands, Tunable-Änderungen mit Tick (8.4), Job-Fertigstellungen mit Tick und Ergebnis (4.5), Trigger-Ereignisse (7.5); dazu je Tick die Metazeile `time` mit `took`, `drift` und `slept` (7.3) — außerhalb der Semantik, der kanonischen Ordnung und der Hashkette, weil Zeitstempel den Trace sonst an die Uhr bänden; das Format ist versioniert (11.3). Der Parametervektor im Kopf ist der zu Beginn des Laufs — Defaults, Profil, Überlagerung einer Kampagne (13.7) —, und `takt replay` wendet ihn an. `takt replay` führt dasselbe Binär im Sim-Modus mit den aufgezeichneten Inputs aus und vergleicht Outputs und Zustandspfade; Abweichung = Fehler in Runtime oder Treiber, nie in der Logik (Satz 9.4.4). **Maschinen-Replay.** `takt replay --machine M [--extract DATEI]` zieht aus dem Gesamtlauf die *Scheibe* von `M`: den Stimulus und alles, was `M` von fremden Maschinen liest — deren Outputs, Zustände, `pub var` und Signale als Zeilen des Traces —, mit dem Kopf `#! maschine M`. Die Scheibe spielt `M` allein ab (nur `M` läuft; die fremden Zeilen gelten als ihre Eingaben, gefolgte Maschinen frisch nach 7.2) und zeigt dieselben Zeilen wie der Gesamtlauf (Satz 9.4.1); `--golden` vergleicht nur die Zeilen von `M`. Ein Feldfehler wird so zum Einzeltest einer Maschine. Die Scheibe gilt ab Tick 0; ein Ausschnitt ab Tick k bräuchte einen Zustands-Schnappschuss der Maschine. **Hashkette.** Der Kopf trägt neben dem Logik-Hash das Kettenende `h_n` der Kette `h_0 = H(Logik-Hash)`, `h_k = H(h_{k-1} ‖ Zeilen des Ticks k)` über den kanonischen Trace (T5, T6 in `grammar/trace.md`); `takt verify-trace TRACE --record R` rechnet sie nach. Ein Dritter mit demselben Binär und denselben Eingaben reproduziert denselben Trace und dieselbe Kette (Satz 9.4.4); wer den Kopf signiert und womit, liegt außerhalb der Sprache.
...
**Gerätemodell in der Simulation: Pflicht.** Ein Port ist im Sim-Build ein Channel-Paar. Gelesen wird der `sim`-Output `mmio/ADR/r`, den ein Modell stellt — mit Unit-Delay wie jeder Modellwert (8.3). Geschrieben wird in den Eingabestrom `mmio/ADR/w`, der jeden Vorgang einzeln und in Reihenfolge führt, auch mehrere je Tick; sichtbar ist ein Element ab dem nächsten Tick, wie das eines internen Stroms (9.6). Läse das Modell den Wert des laufenden Ticks oder sähe es ein Element im selben Tick, hinge beides davon ab, ob es vor oder nach dem Treiber schreitet (Satz 9.4.1). Stellt das Modell den Lesekanal als `stream<Regs>`, entnimmt jedes Lesen ein Element in Sendereihenfolge — auch mehrere je Tick, wie ein Datenregister, das beim Lesen weiterschaltet; ist der Strom leer, liest der Port das zuletzt entnommene Element, vor dem ersten den Default. Ein Pegelkanal bleibt der Default. Damit hängen Treiber und Modell an derselben Adresse, ohne einander zu kennen, und die Adresse bleibt ein Schlüssel in die Konfiguration (8.10) wie jede andere. Ohne Modell liest ein Port den Defaultwert seines Records; das ist kein Fehler, aber auch keine Aussage — die Hardware-Korrektheit eines Treibers ist Sache seiner Modell-Szenarien, nicht des Compilers.
...
**Auf einem endlichen Lauf** hat jede Eigenschaft drei Ausgänge: *erfüllt* (jede Position entschieden wahr), *verletzt* (FAIL-Befund mit der Position der Verletzung, im Trace als `property <name> violated <tick>` in dem Tick, in dem die Fenster der Position geschlossen sind — Position plus Zukunftstiefe der Formel, derselbe Tick im Interpreter und im nativen Monitor) oder *offen am Laufende* (ein Fenster reicht über das Ende hinaus; weder PASS noch FAIL — der Bericht nennt die erste offene Position). `always`/`never` stehen nicht unter einem beschränkten Operator (Prüfung 56), weil die Formel sonst nicht überwachbar wäre; ein Atom, das sich nicht auswerten lässt (ungültiger Input, 3.5), zählt als falsch (13.5). Ein Laufzeitmonitor (`with monitor = true`) verlangt `always(φ)` oder `never(φ)` außen und in φ nur beschränkte Operatoren: Er hält je Atom einen Ring über Vergangenheits- plus Zukunftstiefe der Formel und entscheidet die Position `k − F` im Tick `k` — Kosten je Tick sind die Atome plus die Fenster, statisch im Budget (9.4.3).
...
**Kodierung für `takt prove`.** `takt prove DATEI --export DATEI.smt2 [--depth k]` schreibt die Schrittfunktion als Transitionssystem in SMT-LIB2: je Maschine Blatt, `FAULTED`, Abort-Latch, Aktivierungszähler, Timer je Zustand, Variablen und Signale, je Output der Latch; Ganzzahlen, Enums und Dauern als 64-Bit-Vektoren, `float` als `Float32`/`Float64` der FP-Theorie. Ein Tick ist die symbolische Ausführung des Interpreters (`loop:`-Blöcke der Kette, Übergänge outer-first, Fault-Pfad und `switch`, Abort-Phase, Zähler, Commit); Kanal-Ranges und `assumption`-Formeln sind Annahmen je Tick, `always(φ)`/`never(φ)` ohne Zeitoperatoren sind Invarianten. Die Datei trägt BMC bis zur Tiefe und den Induktionsschritt, je Eigenschaft eine `check-sat`-Anfrage, und nennt ihre Reichweite: Inputs gelten als gültig, `now` und Tunables sind frei bzw. konstant, i64-Überlauf ist nicht modelliert; Ströme, Handler, Jobs, `every`, `at` und Sammlungen kodiert sie nicht, sondern meldet sie. Der Interpreter bleibt die Spezifikation: Das Modell läuft konkret mit denselben Eingaben und stimmt Tick für Tick mit ihm überein (`takt-prove/tests`).

**Beweis mit Solver.** Ohne `--export` ruft `takt prove` einen Solver (`--solver`, `TAKT_SOLVER`, sonst `z3` oder `cvc5` auf dem PATH oder in `~/.takt/bin`, wie `clang` für `takt build`) und gibt je Eigenschaft eines von drei Urteilen: *bewiesen* (BMC bis zur Tiefe ohne Gegenbeispiel und der Induktionsschritt gilt — k-Induktion mit k = Tiefe), *verletzt* mit einem Gegenbeispiel als Stimulus (12.5; `--out DIR` legt ihn als `<name>.stim.trace` ab), das der Interpreter nachgespielt und bestätigt hat — bestätigt er es nicht, meldet der Bericht eine abweichende Kodierung statt eines Fehlers im Programm —, oder *unbewiesen* mit Grund (Induktionsschritt offen, Zeitlimit, `unknown`). Eine wahre, aber nicht k-induktive Eigenschaft bleibt unbewiesen; eine größere Tiefe hilft, wenn ein Timer die Lücke ist. **Prüfstellen-Klassifikation.** Jede `check`-/`expect`-Stelle und jede implizite Prüfung mit Fault-Zweig (4.1: Range, Division durch null, Endlichkeit) ist zugleich ein Beweisziel: Der Bericht klassifiziert sie als *bewiesen unerreichbar* (kein Pfad bis zur Tiefe, und induktiv — die deklarierten Ranges des Zustands gelten dabei als Invarianten, weil ein Wert außerhalb faultet statt gespeichert zu werden), *erreichbar mit Pfad* (Stimulus, im Interpreter bestätigt) oder *unentschieden*. `--save-proof DATEI` schreibt die bewiesen unerreichbaren impliziten Stellen mit dem SHA-256 der Quelle als `.takt-proof`; `takt build --proof DATEI` lässt sie im erzeugten Code aus (Prüfung 65), der Interpreter prüft sie weiter und meldet einen Verstoß als Fehler des Compilers (3.4). Ohne Beweisdatei bleibt das Budget (9.4.3) davon unberührt: Eine bewiesen unerreichbare Prüfung läuft und kostet weiter (FB-49).

**Kompositionalität.** Unter dem Unit-Delay (Entscheidung 6) ist Assume-Guarantee zirkelfrei: Nimmt A in Tick k etwas über B an, ist der Wert von Tick k−1 gemeint, die Induktion läuft also über die Zeit statt über die Komponenten, ohne Fixpunktbildung. Mit `follows` bleibt das gültig, weil die Kanten azyklisch sind (Prüfung 33) — die Induktion folgt dann der topologischen Ordnung innerhalb des Ticks. Praktische Folge: `takt prove` beweist je Maschine, mit den Ψ-Lesevorgängen als freien Variablen unter Annahmen, statt das Gesamtsystem in einen Solver zu werfen. Frei sind dabei auch fremde Outputs und ein fremder `abort`, jeweils unter der Invariante ihres Typs; einen Pfad, den ein Maschinenmodell findet, sucht `takt prove` am Gesamtmodell und bestätigt ihn im Interpreter. Ist das Ganze nicht kodierbar (Handler, Ströme), bleiben die Maschinen ohne solche Konstrukte beweisbar. Von den Kanal-Attributen ist `max_slew` eine Annahme über die Änderung je Tick; `debounce` und `max_age` betreffen die Qualität, und Inputs gelten im Modell als gültig (3.5).
...
scenario "erase interrupted at sector 3" every 1 ms:
    ...
    state RUN:
        sequence:
            ...
            verdict pass
```
Der Name ist ein String; als Maschinenname — in Trace-Zeilen (T1), in `takt test` und für `--scenario` — steht er mit `_` statt Leerraum (`erase_interrupted_at_sector_3`), damit die Felder einer Trace-Zeile getrennt bleiben (FB-167). `takt test` führt jedes Szenario als eigenen Sim-Lauf aus, sammelt Verdikte und Coverage (13.2: Zustände, Transitionen, Checks, Handler); `--out DIR` schreibt je Szenario `NAME.trace` (12.5). Ein Lauf endet, sobald das Szenario einen Zustand ohne Ausgang oder `FAULTED` erreicht hat, sonst mit der Tickgrenze des Aufrufs; ohne Aussage ist er `INCONCLUSIVE` (13.5). Szenarien schreiben nur `sim`-Outputs, die kein Modell stellt (Prüfung 26); zwei Szenarien dürfen denselben Output stellen, weil sie nie zusammen laufen.
...
`takt driver-test <treiber>` führt einen Treiber gegen simulierte Hardware aus und prüft die Verträge aus 12.6 (Zeitstempel, `seq`, Kapazitäten, Qualitätsflags, Ranges, Safe-State bei Heartbeat-Verlust). Mit realer Hardware misst er je Output `guard` und `jitter` (7.5) und je Input die Latenz der Abtastung und schreibt die Werte in die Hardware-Konfiguration, aus der der Compiler seine Annahmen für `at`, Kampagnen und die Latenzformel liest. Für native Funktionen (4.5) prüft die Suite Bit-Gleichheit der Ergebnisse über alle Targets und Panic-Freiheit unter Fuzzing; erst danach wird eine Funktion in die kuratierte Menge aufgenommen. Je Target kalibriert die Suite außerdem die Kostentabelle `c_target[c]` pro Operationsklasse (9.4.3) samt den eigenen Gewichten von Division, `fma` und `sqrt` (7.2) und misst im `rtos`-Profil den Tick-Jitter bei aktivem Funk (12.8); beide Werte gehen mit der Version des Kostenmodells in die Hardware-Konfiguration ein, aus der der Compiler die Schedulability (7.2) prüft. Die Suite prüft außerdem die Subnormal-Behandlung der FPU mit einem Referenzvektor (4.2).

**`takt bench`.** Aussagen über Geschwindigkeit werden gemessen, nicht geschätzt: repräsentative Kerne (PID-Schleife bei 1 kHz, 16-Kanal-Thermoelementprüfung, DFA-Zeilenparsing bei 2 000 Zeilen/s, 4×4-Matrix-Update, CRC32 über 256 Byte, Sequenzschritt mit Fault-Pfad) jeweils als Takt-Programm und als Referenz in C (`clang -O2`, gleiche Semantik inklusive Prüfungen), gemessen je Zielklasse (12.8). Ausgabe: Verhältnis Takt/C je Kern, Anteil der impliziten Prüfungen, kalibrierte `c_target`-Tabelle je Klasse, `T_IO`, Tick-Jitter. Erst diese Tabelle rechtfertigt eine Aussage wie „nahezu C-Geschwindigkeit" — pro Zielklasse und Kern. Die Suite misst außerdem die Stack-Reserven von Runtime, Treibern und ISRs je Profil (Stack-Painting unter Last) und den maximalen Stack-Bedarf jeder nativen Funktion, der als `stack`-Vertrag deklariert wird (4.5, 12.3). **Bericht und Hardware-Konfiguration tragen dieselben Zahlen mit verschiedener Aufgabe, und die Richtung ist festgelegt:** Der Bericht ist das Protokoll der Messung mit ihren Umständen — Datum, Board, Profil, Zahl der Wiederholungen, Streuung —, die Konfiguration trägt das Ergebnis, das der Compiler liest. Der Bericht ist die Quelle, die Konfiguration die Ableitung; `takt bench` schreibt beide in einem Lauf, damit sie nicht auseinanderlaufen können. Wer eine Zahl in der Konfiguration anzweifelt, findet im Bericht, wie sie entstand.
...
| Blockierendes NVM | Stillstand hinnehmen · asynchroner Treiber je Chip · deklarierte Kosten, Schreiben im Schlaf, explizite Freigabe | **deklarierte Kosten (8.10), Prüfung 32, Schreiben im Schlaffenster, `overrun = alert`; asynchroner Treiber als Option mit Residenznachweis** | die Tatsache steht beim Board, das Urteil beim Compiler, die Wahl beim Programm (12.3) |
...
**Der Konfigurationswechsel in einer Reihenfolge (5.2, 5.3, 5.11, 9.3; FB-288).** 5.2 nannte für einen Wechsel `exit` → Aktionen → `enter`, 9.3 führt die Aktionen im Schritt aus, vor `switch`. 9.3 ist die exakte Fassung, und der Interpreter folgt ihr; 5.2 sagt jetzt dasselbe. Die Reihenfolge ist auch die bessere: Die Aktionen laufen in dem Zustand, der den Übergang gewählt hat, ein Fault in ihnen ist dessen Fault, und `exit` läuft genau einmal. Ein Messkern auf dem Board (FB-287) zeigte, dass der erzeugte Code an mehreren Stellen anders ordnete; dabei fielen drei Stellen auf, die die Referenz offen ließ. Erstens sehen `exit`-Blöcke die verlassene Konfiguration: `time_in_state` ist dort die Verweildauer des verlassenen Blatts. Bisher las der Interpreter den Zähler des Ziels, der noch den Stand seines letzten Aufenthalts trug — ein Wert ohne Bedeutung, den kein erzeugter Code nachbilden kann. Zweitens ist `FAULTED` die Senke des Fault-Waldes: Ein Fault, während die Konfiguration `FAULTED` ist, führt dorthin zurück. Der Interpreter ging bisher zum Fault-Ziel der Maschine; scheiterte dort derselbe `exit`-Block wieder, drehte sich `resolve_m` im Kreis, gegen Lemma 9.3.1 — im erzeugten Code wäre das eine Endlosschleife gewesen. Drittens beendet ein Fault im `exit`-Block einer gescopten Instanz nur deren Austritt (5.11); der Interpreter brach bisher den ganzen Lauf ab, der erzeugte Code übersetzte nicht. Programme ohne Fault in einer Aktion und ohne `time_in_state` in einem `exit`-Block ändern ihre Bedeutung nicht.

**Eigene Gewichte für `fma` und `sqrt` (7.2, 9.4.3, 13.8; FB-298).** 7.2 gab der Division eigene Gewichte, weil sie je nach Kern Hardwarebefehl oder Bibliotheksaufruf ist. Für `fma` und `sqrt` gilt dasselbe — 4.2 verlangt sie korrekt gerundet, in Hardware, wo es sie gibt, sonst in Software —, und die Matrizen rechnen jedes Skalarprodukt als `fma`-Kette (3.11). Als gewöhnliche Operation gezählt, lag der Matrix-Kern von `takt bench` auf dem ESP32-C6 ohne FPU beim 5,5-Fachen seiner Schranke, und die Kalibrierung hätte die ganze Tabelle um diesen Faktor strecken müssen, jede Addition mit. Beide zählen jetzt wie die Division in ihrer Klasse und dazu für sich, und `takt bench` misst ihr Gewicht mit eigenen Proben. Die Zeitschranke gewichtet jede solche Operation mit ihrer Klasse und legt das Mehrgewicht darauf; so bleibt das komponentenweise Maximum über Zweige eine Schranke, auch wenn es aus einem Zweig eine Division und aus einem anderen ein `fma` trägt.

**Eine Kalibrierung gehört zu einem Kostenmodell (8.10, 13.8; FB-300).** Die Gewichte einer Kalibrierung sind Zeiten je *gezählter* Operation. Ändert sich, was das Kostenmodell zählt, gehören alte Gewichte zu anderen Zählungen: Seit FB-299 zählt eine Endlichkeitsprüfung als Vergleich von Bits in `i32` statt in ihrer Gleitkommaklasse, und die Tabelle von Board 1 hatte das Gewicht der Prüfungen in ihr Gleitkommagewicht gemittelt — unter dem neuen Modell ergäbe sie für Gleitkommacode eine Schranke von kaum mehr als der Hälfte. Eine zu kleine Schranke ist schlimmer als keine. Die Tabelle trägt darum die Version des Kostenmodells, zu der `takt bench` sie gemessen hat, und der Compiler urteilt nur mit einer Tabelle seiner Version; sonst meldet Prüfung 32, dass neu gemessen werden muss. Was nicht an den Gewichten hängt — Speicher (Prüfung 39), Journal —, prüft er weiter.

**Geplante Ausgaben im Takt der Schritte (9.8, 12.3; FB-301, FB-303).** Zwei Sätze machen ausdrücklich, was die Referenz bisher nur mittelbar sagte. Erstens der Vorrang eines geplanten Werts: 12.1 setzt `apply_scheduled(k)` nach die Schritte und vor den Commit, also gewinnt ein geplanter Wert gegen eine Zuweisung desselben Ticks. Ein Testrahmen und ein Kommentar im Codegen hatten die Anwendung an den Tickbeginn gelegt und damit den umgekehrten Vorrang angenommen, ohne dass ein Test den Fall sah. Zweitens die RAM-Residenz auf XIP-Zielen: Sie gilt auch für die Bibliotheksroutinen, die der übersetzte Code im Tick ruft; ein `fma` aus dem Flash stünde bei asynchronem NVM ebenso still wie der Code, der es ruft.

**Registerports: Unit-Delay in beide Richtungen und die Form auf einem Wirt (12.10; FB-261, FB-307).** 12.10 verlangte für das Lesen eines Ports schon den Unit-Delay eines Modellwerts; der Interpreter las aber den Latch des Modells, also den Wert des laufenden Ticks, wenn das Modell zuvor geschritten war, und stellte ein geschriebenes Element sofort zu. Beides hing damit von der Schrittfolge ab: `68_uart_port` mit vertauschten Maschinen lieferte `busy` einen Tick versetzt, und ein Modell sah dasselbe Element in Tick 2 oder 3, je nachdem, wer zuerst schritt — gegen Satz 9.4.1. Jetzt liest ein Port den committeten Modellwert, und ein Schreibvorgang wird nach allen Schritten zugestellt, sichtbar ab dem nächsten Tick. Dazu die Form auf einem Wirt: Ein Zugriff auf die absolute Adresse bricht dort ab, und der Differenztest hatte das nicht gesehen (FB-305). Jeder Portzugriff geht darum über einen Helfer, dessen Rumpf das Ziel bestimmt — auf einer MCU der `volatile`-Zugriff, eingebettet und im Objekt bitgleich wie zuvor, auf einem Wirt der Aufruf der Runtime. Neben dem Triple ist das der einzige Unterschied der IR zwischen den Zielen.

**Tiefschlaf mit Weckzeit (12.7, 8.10, 12.1; FB-309).** 12.7 sagte, dass `reboot = DEEP_SLEEP` den Lauf beendet und der nächste mit `DEEP_SLEEP_WAKE` beginnt, aber nicht, was den Schlaf beendet. Ohne Weckquelle ist Tiefschlaf ein Aus bis zum Reset, und der nächste Start könnte `DEEP_SLEEP_WAKE` gar nicht melden. Drei Wege standen offen. Ein eigener Kanal `sys/wake_after` hätte zwei Kanäle verlangt, die im selben Tick zueinander passen, und ein vergessener Wert hätte für immer oder gar nicht geschlafen. Eine Weckzeit in der Hardware-Konfiguration hätte eine Entscheidung des Programms — ein Logger, der alle zehn Minuten misst — ans Board gelegt. Gewählt ist das Feld des Kommandos: `DEEP_SLEEP_FOR(duration)` trägt die Weckzeit atomar und typgeprüft wie `EfuseCmd` seine Version, und als neue Variante des offenen Enums `RebootCmd` bricht sie kein Programm (2.5). `DEEP_SLEEP` behält seine Form und sagt jetzt genau, was es heißt: kein Zeitgeber. Die Pins, die wecken, sind die Wake-Quellen, die ein Programm schon für `idle` deklariert (5.10); welche davon ohne RAM arbeiten, ist eine Tatsache des Boards und steht in der Konfiguration (`deep_wake`). Prüfung 60 warnt vor einem `DEEP_SLEEP`, aus dem nichts weckt, und 12.1 nennt den Schritt, an dem die Schleife ein Kommando an die Plattform ausführt.

**Reset-Ursache, Zähler und eine Plattform ohne Startstufe (12.7; M10 Schritt 6b).** 12.7 führte `reset_count` ohne Bedeutung und sagte nicht, was `image_state` auf einer Plattform liefert, die keine Images verwaltet. Für den Zähler standen drei Lesarten offen. Die Starts seit dem Einschalten zählen auch jedes Wecken eines Geräts, das minütlich tief schläft, und sagen nicht, ob die letzten Starts Abstürze waren. Alle Starts im Flash zu zählen kostet einen Schreibvorgang je Start und beantwortet eine Frage der Wartung, die ein Programm mit `persist` selbst beantworten kann. Gewählt sind die Starts in Folge ohne geordnetes Ende: Ein Watchdog nach dem anderen zählt hoch, ein befohlener Neustart, ein Tiefschlaf oder das Einschalten setzt auf 0. Das ist die eine Zahl, die ein Programm nicht selbst halten kann, denn ein Lauf, der vor seinem ersten Journal-Eintrag endet, hinterlässt in `persist` nichts. Dazu die Liste der Reset-Ursachen: `POWER_ON` steht für jeden Reset, der die Plattform von vorn beginnen lässt, auch Reset-Pin und Debugger; `WATCHDOG` nur für den Watchdog der Runtime. Eine Plattform ohne Startstufe hat ein Image, und es ist bestätigt: `image_state` liefert `CONFIRMED`, statt zu fehlen — ein fehlender Wert machte aus `image_state == TRIAL` einen `SensorFault` (3.5), und die Plattform weiß die Antwort.

**Der Watchdog der Schleife (12.3; M10 Schritt 8, FB-296).** 12.3 nannte den Hardware-Watchdog, aber nicht, wann die Schleife ihn bestätigt und wie lang seine Frist ist. Drei Stellen hätten einen echten Watchdog auslösen lassen, ohne dass etwas stand: der Schlaf eines `idle`-Zustands, in dem die Schleife viele Perioden lang keinen Schritt macht; das geordnete Ende eines Laufs, das auf die Leitung wartet; und der Wettlauf zwischen der Prüfung „Tick noch nicht da" und dem Schlafbefehl, der den Tick um eine Periode verschieben konnte. Jetzt bestätigt die Schleife den Watchdog nach dem Schritt und an jeder Tickgrenze, die sie abwartet; die Frist deckt zwei Perioden und einen blockierenden NVM-Vorgang; das Ende hat eine eigene Frist; und Prüfung und Schlaf sind eins. Wo nur ein Reset den Watchdog anhält, gehen Tiefschlaf und Bootloader über einen Reset — der Bootloader bedient ihn nicht, und im Standby zählte er weiter.
