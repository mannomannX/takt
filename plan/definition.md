...
Festlegungen der Umsetzung: Das Element von `fired` ist der Record aus den Captures des Musters und `.t` — derselbe, den eine Handler-Bindung trägt, damit `event.n` im `then` und `f.n` nach `fired as f` dasselbe heißen. Eine Bindung auf `t.fired` trägt ihn direkt, ohne die Hülle, die ein gewöhnlicher Strom bekommt. Der Trigger führt einen eigenen Cursor auf seinem Quellstrom: Er wird mit Ereignisrate ausgewertet und gehört keiner Maschine, kann also nicht den Cursor seines Besitzers benutzen — der liest den Strom in der Regel gar nicht. Im Interpreter liegt die Trigger-Phase zwischen Zustellung und Schrittphase, damit die geplante Ausgabe im selben Tick in `sched` steht; `fired` wird wie jeder interne Strom im nächsten Tick sichtbar (8.6). Prüfung 55 verlangt zusätzlich `bound > 0` und, wo die Zeit die Form `event.t + d` mit konstantem `d` hat, `d >= bound`; `arm` aus einer zweiten Maschine ist ein Fehler, weil `armed` Zustand genau einer ist.
