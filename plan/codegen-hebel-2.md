...
- **Trace `NAME(f1, f2)` im MCU-Rahmen**: Feldtabelle je Variante, Felder aus den 8-Byte-Fächern, verschachtelte Enums mit Namen. Board: Korpus 80 schreibt auf dem C6 `CIRCLE(2.0 )`, `RECT(3.0 , 1.5 )`, `MOVE(10 , -1 )`, `STOP(true)` wie der Interpreter (das Leerzeichen vor `)` kommt von den Zahlhelfern des Boards); ab t ≈ 45 verliert der Trace-Pfad des Bring-ups Bytes (Zeilen verschmelzen), weil Korpus 80 rund 120 Byte je Tick schreibt — mehr, als der USB-Serial-JTAG ohne Rückstau abführt (FB-264). Darum steht 80 nicht im Board-Korpus, bis der Pfad einen Rückstau kennt. UART-Programm (Board, 20 000 Ticks): Der Abbruch bei t = 235 war nicht das Programm, sondern probe-rs — seit dem Überlauf antwortet der JTAG-Teil des USB-Serial-JTAG nicht mehr (DMI-Timeout), und wenn probe-rs nach rund 120 ms aufgibt, reißt es die CDC-Konsole mit. Ohne probe-rs (Reset über RTS, Flash per esptool über die Konsole) läuft dasselbe Abbild durch: 0 verspätet, 0 verloren, `verworfen 0`, Link-Zähler gleich (11 Zeilen, d_rx/d_tx 230). Die Schrittzeit war mit den Ψ-Outputs auf p50 31,3 µs gestiegen (23,5 davor): 14 `memcpy`-Aufrufe je Tick für die Outputs und eine Byteschleife über die 360-Byte-Bank (FB-265). Als typisierte Zuweisungen 27,6 µs, mit der Bank in 8-Byte-Wörtern **17,5 µs (p99 21,8)** — unter dem alten Stand, weil die Byteschleife schon vorher rund 8 µs kostete. `.rwtext` 15 632, `.bss` 34 520, `.text` 32 002.
