        ...
        match cs[i] {
            ...
            '{' => {
                ...
                if let Some(sp) = spec {
                    ...
                    let ok = sp == "hex" || digits(&sp) || (sp.starts_with('.') && digits(&sp[1..]));
                    if !ok {
    ...
    while i < cs.len() {
        match cs[i] {
            ...
            '{' => {
                ...
                let open = i;
                let close = cs[i..].iter().position(|&c| c == '}').ok_or_else(|| err(ErrorCode::Pattern, open, "{ ohne }"))?;
                ...
                let (name, kind) = inner.split_once(':').ok_or_else(|| err(ErrorCode::Pattern, open, "{name:kind} erwartet"))?;
                let name_ok = name.starts_with(|c: char| c.is_ascii_lowercase() || c == '_')
                    && name.bytes().all(|b| b.is_ascii_lowercase() || b.is_ascii_digit() || b == b'_');
                let kind_ok = matches!(kind, "int" | "hex" | "float" | "word" | "str")
                    || kind
                        .strip_prefix("str<")
    ...
    for seg in s.split('/') {
        ...
        let range = if rest.is_empty() {
            ...
            let inner = rest
                .strip_prefix('[')
                .and_then(|r| r.strip_suffix(']'))
                .ok_or_else(|| err(ErrorCode::Address, col + name_len, "Bereich als [a:b]"))?;
            let (a, b) = inner.split_once(':').ok_or_else(|| err(ErrorCode::Address, col + name_len, "Bereich als [a:b]"))?;
            let digits = |t: &str| !t.is_empty() && t.bytes().all(|x| x.is_ascii_digit());
            if !digits(a) || !digits(b) {
                return Err(err(ErrorCode::Address, col + name_len, "Bereich als [a:b]"));
            }
            Some((a.to_string(), b.to_string()))
        };
        out.push(AddressSegment { name: seg[..name_len].to_string(), range });
